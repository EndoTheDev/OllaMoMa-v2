<script setup lang="ts">
  ...
  const isCloud = computed(() => "remote_host" in props.model);

  const modelIcon = computed(() =>
    isCloud.value ? "i-lucide-cloud" : "i-lucide-box"
  );
  ...
  const getTransitionClasses = computed(() => {
    ...
    return {
      ...transitionClasses,
      enterFrom:
        isInfo ? "opacity-0 -translate-x-4" : "opacity-0 translate-x-4",
  ...
  <UAccordion
    ...
    <template #model-details>
      <AppModelsActionsMenu
        ...
        :is-cloud="isCloud"
        @update:active-panel="
          (panel: 'info' | 'modelfile') => $emit('update:activePanel', panel)
        " />
      <div class="relative">
        <Transition
          mode="out-in"
